...
```{r}
attributes(merged_data$hw2a)
```

It is clear from the labels that toilet is considered equivalent to handwash facility.

Following are the types of toilets. We shall consider those structures that fall under MDG as toilets. Households that are not haveing access to MDG toilet will b treated as not having access to toilet.

```{r}
attributes(merged_data$ws8)
```
...
Changing 99 and 9 to `NAs` in `ws8` and `hw3a`


```{r}
merged_data %<>%
  mutate(
    ws8 = ifelse(ws8 == 99, NA, ws8),
    hw3a = ifelse(hw3a == 9, NA, hw3a),
    access_handwas_facility = ifelse(is.na(ws8),NA,
                                     ifelse(ws8 <= 13 | (ws8 >=15 & ws8<= 22)|
                                              ws8 == 31,1,0))
  )

merged_data %>% 
  tabyl(access_handwas_facility, hw3a, show_na = T)
```
...
merged_data %<>%
  group_by(hh_id) %>% 
  mutate(
    temp = access_handwas_facility == 1 & hw3a == 1,
    temp = ifelse(is.infinite(max(temp, na.rm = T)),
                  NA,max(temp, na.rm = T))
  ) %>% 
  ungroup() %>% 
...
merged_data %<>%
  ...
  mutate(
    hh_multigeneration = structure(
      ifelse(is.infinite(min(age, na.rm = T)) | 
               is.infinite(max(age, na.rm = T)) ,NA,
             ifelse(min(age, na.rm = T) < 10 & max(age,na.rm = T) > 60,
             0,1)),
